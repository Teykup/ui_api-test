...
import com.apiTesting.testFramework.controll.ConnectionManager.Method;
...
public class BrandsListDTOTests {
    ...
    class BodyChecksForGetRequestOnBrandsListApi {
        ...
        void checkThatBrandsAreNotListedMoreThanOnce() {
            assertFalse(getBrandsListDTO.checkBrandDuplication(), "There is 1 or more brand not duplications");
        }

        @Test
        @DisplayName("Check that ids are sequential")
        void checkThatIdsAreSequential() {
            assertFalse(getBrandsListDTO.checkForNonSequentialIds(), "There are 1 or more id numbers that are not missing");
        }
    ...
    class HeaderChecksForBrandsListEndpointApiResponsesUsingPutRequest {
        ...
        void checkStatusCodeResponseUsingPutOnBrandsListAPI() {
            assertEquals(200, getResponse.statusCode(), "Status code should return a 200");
        }
